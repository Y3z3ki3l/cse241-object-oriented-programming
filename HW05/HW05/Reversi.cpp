...
ostream& operator<<(ostream& outputStream, const Reversi &currentGame){
    ...
    outputStream << endl;
    for (i = 0; i < currentGame.getHeight(); ++i) {
        if (i < 9)
            outputStream  << " ";
        outputStream << " " << i + 1;
        for (j = 0; j < currentGame.getWidth(); ++j) {
            outputStream << " " << currentGame.gameCells[i][j].getCell();
        }
        if (i == currentGame.getHeight() / 2 - 2)
            outputStream  << "\t    Scoreboard";
        if (i == currentGame.getHeight() / 2 - 1)
            outputStream  << "\t  User  Computer ";
        if (i == currentGame.getHeight() / 2) //board'un yanina skorbord bas
            for (int i = 0; i < currentGame.getHeight(); ++i) {
                for (int j = 0; j < currentGame.getWidth(); ++j) {
                    if (currentGame.gameCells[i][j].getCell() == user)
                        ++countUser;
                    else if (currentGame.gameCells[i][j].getCell() == computer)
                        ++countComputer;
                }
            }
            outputStream << "\t    " << countUser << "  \t   " << countComputer;
        if (i == currentGame.getHeight() / 2 + 1)   //yasayan canli hucre sayisini bas
            outputStream  << "  Number of Living Cell: " << currentGame.countNumberOfLivingCell();
        outputStream << endl;
    }
    return outputStream;
}
Reversi Reversi::operator +=(const Cell &currentMove){
    int coordinateX, coordinateY;
    
    oldMove.push_back(*this);
    
    coordinateX=currentMove.getX();
    coordinateY=currentMove.getY();
    
    play(coordinateX, coordinateY);
    return (*this);
}
Cell Reversi::operator[](const char *nameOfCell){
    bool status=true;
    int coordinateX;
    int coordinateY;
    int temp1, temp2;
    if(nameOfCell[3]=='\0' && isalpha(nameOfCell[2]) && isdigit(nameOfCell[1]) && isdigit(nameOfCell[0])){
        temp1=static_cast<int> (nameOfCell[0]-49)*10;
        temp2=static_cast<int> (nameOfCell[1]-49);
        coordinateY=temp1+temp2;
        coordinateX=static_cast<int> (toupper(nameOfCell[2])-65);
    }
    else if(nameOfCell[2]=='\0' && isalpha(nameOfCell[1]) && isdigit(nameOfCell[0])){
        temp1=static_cast<int> (nameOfCell[0]-49);
        coordinateY=temp1;
        coordinateX=static_cast<int> (toupper(nameOfCell[1])-65);
    }
    else {
        //cout << "Error\n";
        status=false;
    }
    if(coordinateX>=0 && coordinateX<getWidth() &&  coordinateY>=0 && coordinateY<getHeight() && status==true){
        //cout << "correct\n";
        return Cell(coordinateX, coordinateY, emptyCell);
    }
    else{
       // cout << "overflow\n";
        return Cell(-1000,-1000,emptyCell);
    }
}
Cell Reversi::operator()(int coordinateY, char coordinateX){
    char temp = toupper(coordinateX);
    int intCoordinateX=static_cast<int> (temp-65);
    --coordinateY;
    if(intCoordinateX>=0 && intCoordinateX<getWidth() &&  coordinateY>=0 && coordinateY<getHeight()){
        //cout << "correct\n";
        char c=gameCells[coordinateY][intCoordinateX].getCell();
        return Cell(intCoordinateX, coordinateY, c);
    }
    else{
        //cout << "overflow\n";
        return Cell(-1000,-1000,emptyCell);
